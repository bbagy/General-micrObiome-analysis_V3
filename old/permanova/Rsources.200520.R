...
pairwise.adonis <- function(x,factors, map,mvar, sim.function = 'vegdist', sim.method = 'bray', adjust,p.adjust.m ='bonferroni',reduce=NULL)
{
  ...
  pairw.res <- data.frame(pairs,Df,SumsOfSqs,R2,p.value,p.adjusted,sig)
...
pairwise.adonis.ad <- function(x,factors, map,mvar, sim.function = 'vegdist', sim.method = 'bray', adjust=NULL,p.adjust.m ='bonferroni',reduce=NULL)
{
  set.seed(1)
  co <- combn(unique(as.character(map[,mvar])),2)
  pairs <- c()
