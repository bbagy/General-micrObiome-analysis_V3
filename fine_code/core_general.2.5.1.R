...
Go_barchart <- function(psIN, metaData, project, taxanames, data_type, simple = "no",  x_label="SampleIDfactor", facet=NULL, legend="bottom", orders,cutoff=0.005, name=NULL, ncol=11, height, width,plotCols,  plotRows){
  ...
  if (!is.null(facet)) {
    if (!is.null(name)) {
      pdf(sprintf("%s_%s/pdf/3_barchart.%s.%s.%s.%s.(%s).%s.pdf",project, format(Sys.Date(), "%y%m%d"),project, facet,name, cutoff, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }
    else {
...
#des = des
#alpha = 0.05
Go_deseq2 <- function(psIN, metaData, project,order,type, data_type, adjust, des, name, alpha=0.05){
...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                  Version: Go.2.5.2 \n"))
cat("                                              Write by Heekuk \n")
cat(yellow("Automated installation for the required packages and read libraries.\n"))
cat(blue("#--------------------------------------------------------------# \n"))
