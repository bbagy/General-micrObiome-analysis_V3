...
Go_cluster <- function(psIN, project,db, percent){
  ...
  #----- re classification  -----#
  db <- "/Users/heekukpark/DB/DADA2/Bacteria/silva_nr99_HOMD.fasta.gz"
...
Go_barchart <- function(psIN, metaData, project, taxRanks, data_type, simple = "no", x_label, facet, legend, orders, alpha, name, ncol, height, width,plotCols,  plotRows){
  ...
  if (length(facet) == 1) {
    if (length(name) == 1) {
      pdf(sprintf("%s_%s/pdf/3_barchart.%s.%s.%s.%s.(%s).%s.pdf",project, format(Sys.Date(), "%y%m%d"),project, facet,name, alpha, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }
    else {
    ...
    if (length(name) == 1 ) {
      pdf(sprintf("%s_%s/pdf/3_barchart_simple.%s.%s.(%s).%s.pdf",project, format(Sys.Date(), "%y%m%d"),project,name,alpha, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }      else {
  ...
  for(i in 1:length(taxaname)){
    ...
    p <- ggplot(df2, aes_string(x= x_label, y="value", fill=taxaname[i], order=taxaname[i])) + geom_bar(stat="identity", position="stack") + theme_classic()  + labs(fill=taxaname[i])+
      ...
      guides(fill=guide_legend(ncol= col))  + guides(col = guide_legend(ncol = col)) + 
...
Go_barchart_spcies <- function(psIN, metaData, project, data_type,simple, x_label, facet, legend, orders, alpha, name,ncol, height, width,plotCols, plotRows){
    if(!is.null(dev.list())) dev.off()
  colorset = "Set1" # Dark1 Set1 Paired
  ...
  if (length(facet) == 1) {
    ...
    if (length(name) == 1) {
      pdf(sprintf("%s_%s/pdf/3_barchart_species_simple.%s.%s.(%s).%s.pdf",project, format(Sys.Date(), "%y%m%d"),project,name,alpha, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    } else {
  ...
  p <- ggplot(df2, aes_string(x= x_label, y="value", fill="Genus.Species", order="Genus.Species")) + geom_bar(stat="identity", position="stack") + theme_classic()  + labs(fill=NULL)+
    theme(legend.position=legend, legend.text=element_text(size=8), axis.title.x = element_blank(), axis.text.x = element_text(angle=90, vjust=0.5, hjust=1, size=8)) + 
    guides(fill=guide_legend(ncol= col))  + guides(col = guide_legend(ncol = col)) + 
    ylim(c(-.1, 1.01)) + scale_fill_manual(values = getPalette(colourCount)) + labs(y = "Relative abundance")
...
Go_box_plot <- function(df, metaData, project, orders=NULL, outcomes, Chao1_Normality="no", Shannon_Normality="no" ,plot="box", star="no",title= NULL, facet= NULL, name= NULL, xanlgle=90,  height, width, plotCols, plotRows){
  ...
  for (mvar in rownames(subset(metadata, Go_clme == "yes"))) {
    ...
    for (des in unique(adiv[,mvar])){
      ...
      for (am in alpha_metrics){
        ...
        # plot
        p <- ggplot(adiv[adiv[,mvar]==des,], mapping = aes_string(x=timepoint, y=am, color=timepoint, group=paired)) + geom_line(color="grey") + geom_point(size = 1.25) + xlab(timepoint) + ylab(sprintf("%s Index\n", am)) + ggtitle(sprintf("%s-%s (%s) ", mvar, des, clme.globalp))  + scale_color_brewer(palette=colorset)+theme_bw() +theme(title=element_text(size=8), axis.text.x=element_text(angle=xangle,hjust=1,vjust=0.5)) + theme(legend.position= "NONE" )
...
Go_linear <- function(df, metaData, project, outcomes,maingroup, orders, name, height, width, plotCols, plotRows){
  ...
  for (mvar in rownames(subset(metadata, Go_linear =="yes"))) {
    ...
    for(i in 1:length(outcomes)){
      ...
      print(outcomes[i])
      p<- ggplot(adiv.na.na, aes_string(x=mvar, y=outcomes[i], group= maingroup, color=maingroup, linetype = maingroup))+
...
Go_bdiv <- function(psIN, metaData, project, orders, distance_metrics, plot,legend, shapes,ellipse, ID, facet, name, height, width){
...
#des = des
#alpha = 0.05
Go_deseq2 <- function(psIN, metaData, project,order,type, data_type, adjust, des, name, alpha){
...
Go_deseq2_volc <- function(project, file_path,files, type,alpha,beta, name, height, width){
  ...
  for (fn in 1:length(filenames)) {
    ...
    dircolors <- c("#4f86f7", "grey","#f7022a"); names(dircolors) <- c(as.character(basline), "NS", as.character(smvar))
    ...
    p1 <- p1 + ggtitle(sprintf("%s, %s vs %s (p < %s,cutoff=%s) ", mvar, basline, smvar,  alpha, beta)) 
    # p1 <- p1 + ggtitle(sprintf("%s (p < %s,cutoff=%s) ", mvar, alpha, beta)) 
    print(p1)
  } 
  dev.off()
}
...
Go_deseq2_fore <- function(project,file_path, files,type, alpha, beta,font, name, height, width){
  ...
  for (fn in 1:length(filenames)) {
    ...
    dircolors <- c("#4f86f7", "#f7022a", "grey"); names(dircolors) <- c("down", "up", "NS")
    p1 <- ggplot(resSig.top, aes(x=reorder(taxa,log2FoldChange), y=log2FoldChange, color=dir)) + 
      ...
      geom_errorbar(aes(x=taxa, ymin=log2FoldChange-lfcSE, max=log2FoldChange+lfcSE), width=0.2)  +  scale_color_manual(values=dircolors) + ylim(c(-lims, lims))+
      xlab("Taxa") + ylab("log2FoldChange")+theme(text = element_text(size=font), 
    ...
    # p1 <- p1 + ggtitle(sprintf("%s %s vs %s (p < %s,cutoff=%s) ", sn, basline, smvar,  alpha,beta)) 
    p1 <- p1 + ggtitle(sprintf("%s, %s vs %s (p < %s,cutoff=%s) ", mvar, basline, smvar,  alpha, beta)) 
  ...
  if (dim(resSig)[1] >= 1) {
    ...
    if (groupby == "smvar"){
      p <- ggplot(resSig.top, aes(x=reorder(taxa,log2FoldChange), y=smvar, color=smvar)) + theme_classic()+ coord_flip() + #x=reorder(taxa,Estimate); 원래 x=factor(taxa). 값에 따라 정열 하기 위해x=reorder(taxa,Estimate)를 사용함
        ...
        scale_fill_gradient2(low = "#4f86f7", mid = "yellow", high = "#f7022a")+ #  c("#4f86f7", "#f7022a", "grey");
...
Go_deseq2_ntd <- function(psIN, project,metaData, taxRanks, adjust, des,name,  order, data_type,alpha){
  ...
  for(t in 1:length(taxaname)){
    ...
    for (mvar in rownames(subset(metadata, Go_deseq2=="yes"))) {
      ...
      map.sel.sel[,mvar] <- factor(map.sel.sel[,mvar])
      cbn <- combn(x = levels(map.sel.sel[,mvar]), m = 2)
      ...
      for(i in 1:length(my_comparisons)){
        ...
        #-- give taxa name --#
        print("pass3")
        res <- cbind(as(tmp, "data.frame"), as(funTab[rownames(tmp), ], "matrix"))
        ...
        if (length(des) == 1) {
          if (length(name) == 1) {
            write.csv(res, quote = FALSE,col.names = NA,file=sprintf("%s/(%s.vs.%s).%s.%s.%s.%s.%s.Deseq2.csv",out_ntd,basline,smvar, mvar, des, name, taxaname[t], project,sep="/"))
          } else {
            write.csv(res, quote = FALSE,col.names = NA,file=sprintf("%s/(%s.vs.%s).%s.%s.%s.%s.Deseq2.csv",out_ntd,basline,smvar,mvar,des,taxaname[t],project,sep="/"))
          }
        } else {
          if (length(name) == 1) {
            write.csv(res, quote = FALSE,col.names = NA,file=sprintf("%s/(%s.vs.%s).%s.%s.%s.%s.Deseq2.csv",out_ntd, basline,smvar,mvar,name,taxaname[t],project,sep="/"))
          } else{
            write.csv(res, quote = FALSE,col.names = NA,file=sprintf("%s/(%s.vs.%s).%s.%s.%s.Deseq2.csv",out_ntd,basline, smvar,mvar, taxaname[t], project, sep="/"))
          }
        }
      }
      ...
      sub.res <- subset(res.ntd, padj < alpha);dim(sub.res)
...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                  Version: core_general.2.5.0 \n"))
cat("                                              Write by Heekuk \n")
cat(yellow("Automated install for the required packages and read libraries.\n"))
cat(blue("#--------------------------------------------------------------# \n"))
