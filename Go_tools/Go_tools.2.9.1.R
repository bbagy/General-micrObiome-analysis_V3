...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.2.9.1 \n"))
cat("                                              Write by Heekuk \n")
...
Go_bdiv <- function(psIN, metaData, project, orders, mycols=NULL,
...
Go_bdivcbn <- function(psIN, metaData, project, orders, distance_metrics, mycols=NULL, 
...
Go_DA_volc <- function(project, file_path,files, type,alpha,beta,mycols=NULL, name, overlaps=10, font, height, width){
  ...
  for (fn in 1:length(filenames)) {
   ...
   if(!is.null(mycols)){
    ...
    if(type == "taxonomy" | type == "taxanomy" |type == "bacmet" ){
      p1 <- p1 +  geom_text_repel(aes(label=ifelse(ShortName != "NA" & df.na$padj < alpha & abs(df.na$log2FoldChange) > beta, as.character(ShortName),'')), size=font, segment.alpha = 0.25, fontface="italic",max.overlaps = overlaps )
    }else if(type == "function"){
      p1 <- p1 +  geom_text_repel(aes(label=ifelse(KOName != "NA" & df.na$padj < alpha & abs(df.na$log2FoldChange) > beta, as.character(KOName),'')), size=font,max.overlaps = overlaps)
    }
...
Go_DA_fore <- function(project,file_path, files,type, alpha, beta, mycols=NULL, font, name, height, width){
