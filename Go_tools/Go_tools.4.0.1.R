...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.4.0.1 \n"))
cat("                                              Write by Heekuk \n")
...
Go_getSigASVs <- function(project, file_path, files= ".csv", ASVs,
                          target.column,
                          target.group){
  ...
  tab <- read.csv(ASVs,row.names=1,check.names=FALSE);dim(tab)
  tab.sel <- tab[unique(tabSig$taxa),] 
  
  write.csv(tab.sel,quote = FALSE,col.names = NA, file=sprintf("%s/%s.%s.%s.csv",out_path,project, "sigASVs",format(Sys.Date(), "%y%m%d"),sep="/"))
}
#' A Go_groupBox
#'
