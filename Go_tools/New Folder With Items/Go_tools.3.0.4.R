...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.0.4 \n"))
cat("                                              Write by Heekuk \n")
...
Go_dualYplot <- function(df, TaxTab, cate.vars, project,  Box, Line1, Line2=NULL,
  ...
  for (mvar in cate.vars) {
    ...
    p1 <- p + geom_line(data = mean.line1.melt, 
      ...
      geom_text_repel(data = label.line1.sel, aes(x = !!sym(mvar), y = value,label = variable, segment.color = 'grey70'),
    ...
    if (!is.null(Line2)){
      ...
      p1 <- p + geom_line(data = mean.line.melt, 
        ...
        geom_text_repel(data = label.line1.sel, aes(x = !!sym(mvar), y = value,label = variable,  segment.color = 'grey70'),
                        size=3, fontface="italic")  +
        geom_text_repel(data = label.line2.sel, aes(x = !!sym(mvar), y = value,label = variable,  segment.color = 'grey70'),
                        size=3, fontface="italic") 
