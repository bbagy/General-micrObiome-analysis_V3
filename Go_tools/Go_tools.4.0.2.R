...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.4.0.2 \n"))
cat("                                              Write by Heekuk \n")
...
Go_alluvialplot <- function(project, 
  ...
  for(target in targets.bac){
    ...
    if (target == "Gardnerella_vaginalis" ){
      mycols <- Go_myCols(custumCols="cols2")
    }else{
    ...
    if (!is.null(column1) & !is.null(column2)){
      p <- ggplot(data = taxaTab.melt, aes(y = value, axis1 = variable, axis2 = !!sym(column1), axis3 = !!sym(column2), axis4 = !!sym(outcome)))
    } else if (!is.null(column1) | !is.null(column2)){
      column = ifelse(!is.null(column1), column1, column2)
      p <- ggplot(data = taxaTab.melt, aes(y = value, axis1 = variable, axis2 = !!sym(column), axis3 = !!sym(outcome)))
    } else {
      p <- ggplot(data = taxaTab.melt, aes(y = value, axis1 = variable, axis2 = !!sym(outcome)))
    }
    ...
    p1 <- p + theme_void() +
      ...
      geom_stratum(width = 5/12, fill = "grey", color = "white") +
...
Go_rfrocplot <- function(rf, 
  ...
  ggroc <- ggplot(df, aes(d = known.truth, m = predictor, color = model)) + geom_roc(n.cuts = 0,size=0.7) +
    theme(panel.grid = element_blank(),  panel.background = element_rect(fill = "white", colour = "Black",size = 0.7, linetype = "solid"), 
          aspect.ratio = 1) # remove legend title
  ...
  # AUC (area under ROC )
  aucTab <- calc_auc(ggroc);aucTab$AUC[1]
  annotate <- ggroc + theme(plot.title = element_text(hjust = 0,size = 10), legend.position = c(0.75, 0.17)) +
    ...
    annotate("text", x = 0.75, y = 0.4, label = paste("Train_set AUC =", round(aucTab$AUC[1], 3), "\n","Test_set AUC =", round(aucTab$AUC[2], 3) ),size = 2.5) +geom_abline(intercept = 0, slope = 1, color = "darkgrey", linetype = "dashed")
...
Go_rfproximityplot <- function(rf, 
  ...
  proximity <- ggplot(tsne_data, aes(x = tsne_x, y = tsne_y, color = label)) + 
    geom_point(size = 1, alpha = 0.8) + ggtitle("Proximity Plot (t-SNE)") + labs(x = "t-SNE 1", y = "t-SNE 2")+ 
    theme(panel.grid = element_blank(),  panel.background = element_rect(fill = "white", colour = "Black",size = 0.7, linetype = "solid"), 
          aspect.ratio = 1) +
    geom_vline(xintercept = 0, size = 0.1) + geom_hline(yintercept = 0, size = 0.1) 
...
Go_rfimportanceplot <- function(rf, 
  ...
  importance <- ggplot(importance_data, aes(x = reorder(Feature, Importance), y = Importance)) +
    geom_bar(stat = "identity", fill = "steelblue") + coord_flip() + 
    xlab("Feature") + ylab("Mean Decrease in Gini") + ggtitle("Variable Importance") + 
    theme(panel.grid = element_blank(),  panel.background = element_rect(fill = "white", colour = "Black",size = 0.7, linetype = "solid"), 
