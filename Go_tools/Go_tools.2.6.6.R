...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.2.6.6 \n"))
cat("                                              Write by Heekuk \n")
...
Go_emptyMap <- function(psIN, project){
  ...
  analysis <- c("type",	"baseline",	"Go_barchart", 	"Go_overview",	"Go_box",	"Go_linear","Go_clme", "Go_reg",	"Go_regConfounder", "Go_bdiv",	"Go_perm",	"Go_deseq2","Go_lmem","Go_lmemConfounder")
...
Go_rare <- function(physeq_object, step = 10, label = NULL, color = NULL, xlimit, ylimit,plot = TRUE, parallel = FALSE, se = TRUE) {
  out <- file.path(sprintf("%s_%s",project, format(Sys.Date(), "%y%m%d"))) 
  if(!file_test("-d", out)) dir.create(out)
  out_path <- file.path(sprintf("%s_%s/pdf",project, format(Sys.Date(), "%y%m%d"))) 
  if(!file_test("-d", out_path)) dir.create(out_path)
  ...
  p <- p + xlim(NA, xlimit) + ylim(NA, ylimit) + theme_classic()
...
Go_box_plot <- function(df, metaData, project, orders=NULL, outcomes,
  ...
  for (mvar in rownames(subset(metadata, Go_clme == "yes"))) {
    ...
    if (mvar == timepoint){
      for (am in alpha_metrics){
        ...
        # plot
        p <- ggplot(adiv, mapping = aes_string(x=timepoint, y=am, color=timepoint, group=paired)) + geom_line(color="grey") + geom_point(size = 1.25) + xlab(timepoint) + ylab(sprintf("%s Index\n", am)) + ggtitle(sprintf("%s \n (%s) ", mvar, clme.globalp))  + scale_color_brewer(palette=colorset)+theme_bw() +theme(title=element_text(size=8), axis.text.x=element_text(angle=xangle,hjust=1,vjust=0.5)) + theme(legend.position= "NONE" )
...
Go_bdiv <- function(psIN, metaData, project, orders, distance_metrics, plot="PCoA", shapes = NULL, ID = NULL, ellipse="yes", facet=NULL, name=NULL, height, width){
  ...
  if (!is.null(name)) {
    if (!is.null(facet)) {
      pdf(sprintf("%s_%s/pdf/6_ordi.%s.%s.%s.%s.pdf",project,format(Sys.Date(), "%y%m%d"),project, facet, name, format(Sys.Date(), "%y%m%d")), height = height, width = width)
      
    } else{
      pdf(sprintf("%s_%s/pdf/6_ordi.%s.%s.%s.pdf",project,format(Sys.Date(), "%y%m%d"),project,name,format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }
  }else{
    if (!is.null(facet)) {
      pdf(sprintf("%s_%s/pdf/6_ordi.%s.%s.%s.pdf",project,format(Sys.Date(), "%y%m%d"),project, facet, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }else{
      pdf(sprintf("%s_%s/pdf/6_ordi.%s.%s.pdf",project,format(Sys.Date(), "%y%m%d"),project,format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }
  }
  ...
  for (mvar in rownames(subset(metadata, Go_bdiv =="yes"))) {
    ...
    for(distance_metric in distance_metrics){
      ...
      p = p + facet_wrap(~ method, scales="free") + theme_bw() # open(1), cross(10), closed(2)
...
#des = des
#alpha = 0.05
Go_deseq2_V2 <- function(psIN, metaData, project, order,type, taxanames, data_type, adjust, des, name, alpha=0.05){
  ...
  # 최근 버전 for unstrafied (20210112 확인)
   if(type == "function"){
    # remove colume sum 0 and psIN 재구성(20201027)
    a <- data.frame(otu_table(psIN))*1000000
    a.ceiling <- ceiling(a[-c(99),])
    b <- a.ceiling[, -which(numcolwise(sum)(a.ceiling) < 1)]
    if (length(b) == 0){
      OTU.sta <- otu_table(a, taxa_are_rows = TRUE);head(OTU.sta)
...
Go_deseq2_volc <- function(project, file_path,files, type,alpha,beta, name, height, width){
  ...
  # out file
  if (!is.null(name)) {
    pdf(sprintf("%s_%s/pdf/7_1_deseq2.volcano.%s.%s.(%s.%s).%s.pdf", project, format(Sys.Date(), "%y%m%d"), project, name, alpha,beta,format(Sys.Date(), "%y%m%d")), height = height, width = width)
  }   else {
      pdf(sprintf("%s_%s/pdf/7_1_deseq2.volcano.%s.(%s.%s).%s.pdf", project, format(Sys.Date(), "%y%m%d"), project, alpha, beta, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }
...
Go_deseq2_fore <- function(project,file_path, files,type, alpha, beta,font, name, height, width){
  ...
  # out file
  if (!is.null(name)) {
    pdf(sprintf("%s_%s/pdf/7_2_deseq2.forest.%s.%s.(%s.%s).%s.pdf", project, format(Sys.Date(), "%y%m%d"), project, name, alpha,beta,format(Sys.Date(), "%y%m%d")), height = height, width = width)
  }  else {
      pdf(sprintf("%s_%s/pdf/7_2_deseq2.forest.%s.(%s.%s).%s.pdf", project, format(Sys.Date(), "%y%m%d"), project, alpha,beta, format(Sys.Date(), "%y%m%d")), height = height, width = width)
    }
  ...
  if (dim(resSig)[1] >= 1) {
    ...
    if (groupby == "smvar"){
      p <- ggplot(resSig.top, aes(x=reorder(taxa,log2FoldChange), y=smvar, color=smvar)) + theme_classic()+ coord_flip() + #x=reorder(taxa,Estimate); 원래 x=factor(taxa). 값에 따라 정열 하기 위해x=reorder(taxa,Estimate)를 사용함
        ...
        scale_fill_gradient2(low = "red", mid = "white", high = "darkblue")+ #  c("#4f86f7", "#f7022a", "grey");
