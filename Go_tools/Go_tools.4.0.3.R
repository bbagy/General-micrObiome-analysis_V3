...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.4.0.3 \n"))
cat("                                              Write by Heekuk \n")
...
Go_rfCMplot <- function(rf, 
  ...
  if(class(tt)== "try-error"){
    ...
    p.confusion <- ggplot(data =  confusionTab, mapping = aes(x = Actual, y = Predicted)) +
      ...
      theme(plot.title = element_text(size = 10, hjust = 0), legend.position = "none") +
      ggtitle(paste("Cross-Validated (5 fold) Confusion Matrix\n Prediction Accuracy (test set): ", Accuracy,"% \n","Out-of-Bag (OOB) error rate: ", oob_error,"%",sep="")) +
      theme(panel.grid = element_blank(),  
            panel.background = element_rect(fill = "white", colour = "Black",size = 0.7, linetype = "solid"), 
            aspect.ratio = 0.5/1)
...
Go_rfproximityplot <- function(rf, 
  ...
  ggplot(tsne_data, aes(x = tsne_x, y = tsne_y, color = label)) + 
