...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.5.1 \n"))
cat("                                              Write by Heekuk \n")
...
Go_regression <- function(data, project,  
  ...
  if(!is.null(cate.vars) & is.null(con.vars)){
    ...
    varis <- cate.vars
  } else if(!is.null(con.vars) & is.null(cate.vars)){
    for (mvar in  con.vars) {
      # NA 제거
    ...
    for (mvar in varis) {
      ...
      if (mul.vars == TRUE){
        if (!is.null(interaction)){
          ...
          mul.form <- sprintf("%s ~ %s", outcome, paste(setdiff(varis, "SampleType"), collapse="+"))
