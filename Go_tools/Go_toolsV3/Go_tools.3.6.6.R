...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.6.6 \n"))
cat("                                              Write by Heekuk \n")
  ...
  for (fn in 1:length(filenames)) {
   ...
   if(!is.null(mycols)){
    ...
    if(plot == "volcano" |  plot == "maplot"){
      p1 <- p1 + theme_bw() + scale_color_manual(values=dircolors, labels=legend.labs) +
      theme(text = element_text(size=font+8),plot.title = element_text(size=font+8), legend.text=element_text(size=font+8),  legend.position="bottom",legend.justification = "left",legend.box = "vertical")  +
...
Go_adiv <- function(psIN, project, alpha_metrics){
  ...
  print("You can measure Observed, Chao1, ACE, Shannon, Simpson, InvSimpson, Fisher")
  # adiv table
  mapping.sel <- data.frame(sample_data(psIN))
