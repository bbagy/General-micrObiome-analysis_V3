...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.6.7 \n"))
cat("                                              Write by Heekuk \n")
...
Go_dist <- function(psIN, project, name=NULL, distance_metrics){
  # out dir
  out <- file.path(sprintf("%s_%s",project, format(Sys.Date(), "%y%m%d"))) 
  if(!file_test("-d", out)) dir.create(out)
  out_path <- file.path(sprintf("%s_%s/table",project, format(Sys.Date(), "%y%m%d"))) 
  ...
  for (distance_metric in distance_metrics) {
    ...
    write.csv(df, quote = FALSE,col.names = NA, sprintf("%s/distance2.%s.%s.%s%s.csv", out_dist, 
                                                                       project, 
                                                                       distance_metric, 
                                                                       ifelse(is.null(name), "", paste(name, ".", sep = "")), 
                                                                       format(Sys.Date(), "%y%m%d")),sep="/")
