...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.7.3 \n"))
cat("                                              Write by Heekuk \n")
   ...
   if(!is.null(mycols)){
    ...
    if(plot == "volcano"){
      ...
      p1 <- ggplot(resSig.top, aes(x=reorder(taxa,log2FoldChange), y=log2FoldChange, color=deseq2)) + 
        ...
        scale_color_manual(values=dircolors, labels=legend.labs, drop = FALSE) + scale_shape_manual(values = ancomshape) + 
        #guides(shape = "none") +
...
Go_correlation <- function(psIN, project,
  ...
  #=======================
  #      For Function table(ps)
  #======================
  if(!is.null(top)){
    Top = names(sort(taxa_sums(psIN), TRUE)[1:top])
    psIN.top = prune_taxa(Top, psIN);psIN.top
  }else{
    psIN.top = psIN
  }
  ...
  pdf(sprintf("%s/Correlation_%s.%s.%s.%s%s.pdf",
              out_path,
              rank,
              method,
              maingroup,
