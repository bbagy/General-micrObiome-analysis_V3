...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.7.2 \n"))
cat("                                              Write by Heekuk \n")
...
Go_DA_plot <- function(project, file_path,files, type="taxonomy", plot = "volcano", fdr, fc, mycols=NULL, name, overlaps=10, font, height, width){
  ...
  for (fn in 1:length(filenames)) {
   ...
   if(!is.null(mycols)){
    ...
    ancomshape <- c(16,1); names(ancomshape) <- c(TRUE, FALSE)# 16,1,1
    ...
    if(plot == "volcano"){
      ...
      p1 <- ggplot(resSig.top, aes(x=reorder(taxa,log2FoldChange), y=log2FoldChange, color=deseq2)) + 
        ...
        theme(panel.background = element_rect(fill = "white", colour = "grey50"),aspect.ratio = 1/0.7)
    ...
    if(plot == "volcano" |  plot == "maplot"){
      p1 <- p1 + scale_color_manual(values=dircolors, labels=legend.labs) + theme(text = element_text(size=font+8),plot.title = element_text(size=font+8), legend.text=element_text(size=font+8),  legend.position="bottom",legend.justification = "left",legend.box = "vertical")  +
        geom_point(aes(shape=ancom), size=font-1.5) + scale_shape_manual(values = ancomshape) +
        theme(panel.background = element_rect(fill = "white", colour = "grey50"),aspect.ratio = 1/1.5)
      # + theme_bw()
  ...
  for (mvar in cate.vars) {
   ...
   print(sprintf("##-- %s (total without NA: %s/%s) --##",
    ...
    if (!is.null(cate.conf) | !is.null(cont.conf)) {
      ...
      out <- ancombc(phyloseq = psIN.cb, p_adj_method = "holm", lib_cut = 1000, 
      ...
      out <- ancombc(phyloseq = psIN.cb, p_adj_method = "holm", lib_cut = 1000, # prv_cut = 0.10, 
                     formula = mvar, 
                     group = mvar, struc_zero = TRUE, neg_lb = TRUE, tol = 1e-5, 
                     max_iter = 100, conserve = TRUE, alpha = 0.05, global = TRUE)
    }
    ...
    rownames(res.ancom.df) <- res.ancom.df$lfc.taxon 
  ...
  for (mvar in cate.vars) {
    ...
    if (!is.null(combination)){
      ...
      for(distance_metric in distance_metrics){
        ...
        p  <- p  + theme(panel.background = element_rect(fill = "white", colour = "grey50"),aspect.ratio = 1)
        print(p)
      }
    }
  }
    ...
    if (!is.null(combination)){
      ...
      for(i in 1:length(group_comparisons)){
        ...
        for(oc in outcomes){
          ...
          p1 <- ggplot(df.cbn, aes_string(x=mvar, y=oc, colour=mvar))  + labs(y=oc, x=NULL) + #theme_bw() + 
            theme(strip.background = element_blank()) +
            theme(text=element_text(size=9), axis.text.x=element_text(angle=xangle,hjust=1,vjust=0.5),
                  plot.title=element_text(size=9)) # ,face="bold"
      ...
      for(oc in outcomes){
        ...
        p1 <- ggplot(df.na, aes_string(x=mvar, y=oc, colour=mvar))  + labs(y=oc, x=NULL) +#  theme_bw() +
          theme(strip.background = element_blank()) +
          theme(text=element_text(size=9), axis.text.x=element_text(angle=xangle,hjust=1,vjust=0.5),
                plot.title=element_text(size=9)) #,face="bold"  
        ...
        if (length(unique(df.na[,mvar])) < 5){
          if(standardsize==TRUE){
            num.subgroup <- length(unique(df.na[,mvar]))*0.2
          }else{
            num.subgroup <- 1
          }
        }else{
        ...
        p1  <- p1  + theme(panel.background = element_rect(fill = "white", colour = "grey50"), aspect.ratio = 1/num.subgroup)
        plotlist[[length(plotlist)+1]] <- p1 
      }
    }
  }
    ...
    if (mvar == timepoint){
      ...
      for (des in unique(adiv[,mvar])){
        ...
        if (height*width <= 6){
          ...
          p  <- p  + theme(panel.background = element_rect(fill = "white", colour = "grey50"),aspect.ratio = 1/num.subgroup)
          plotlist[[length(plotlist)+1]] <- p
        }
      }
    }
...
Go_groupBox <- function(psIN, mainGroup, project, 
  ...
  # plot size ratio
  if (length(unique(df.sel.melt.clean$variable)) < 5){
    if(standardsize==TRUE){
      num.subgroup <- length(unique(df.sel.melt.clean$variable))*0.2
    }else{
      num.subgroup <- 1
    }
  }else{
...
library("yarrr")
  ...
  if(is.null(custumCols) & is.null(RColorBrewer) & is.null(piratepal)){
    ...
    cols3 <- c("#323337FF","#FF0000", "#009E73", "#0000FF", "#FFFF00", "#00FFFF", "#FF00FF", "#C0C0C0", "#808080", "#800000", "#008000", "#000080", "#808000", "#800080", "#E69F00",  "#FFC0CB", "#DC143C", "#ADFF2F", "#00FF7F", "#00CED1", "#9400D3", "#8B008B", "#1E90FF", "#FFD700", "#FF69B4", "#FF6347", "#4169E1", "#FFA07A", "#FF8C00", "#B3DE69", "#20B2AA")
...
Go_pairedperm <- function(psIN, cate.vars, project, distance, distance_metrics, cate.conf=NULL, des, name=NULL){
  # out dir
  out <- file.path(sprintf("%s_%s",project, format(Sys.Date(), "%y%m%d"))) 
  if(!file_test("-d", out)) dir.create(out)
...
print("Check the psIN")
 ...
 # get taxa names
  ...
  if (!is.null(group2) & is.null(group3)){
    ...
    #group2.col <- Go_myCols(piratepal = "pony")
    
    group2.col <- c("#6351A0FF", "#FEF79EFF", "#EB5291FF",  "#9DDAF5FF", "#F5BACFFF",  "#1794CEFF", "#FBBB68FF", "#972C8DFF", "#ECF1F4FF")
    ...
    if (length(unique(mapping.sel[,group1])) > 8){
      getPalette = colorRampPalette(brewer.pal(9, "Set1"))
      group1.col <- getPalette(length(unique(mapping.sel[,group1])))
      names(group1.col) = levels(as.factor(mapping.sel[,group1])) #       names(Path_col) = levels(annotation_row$Path)
      print(1)
    } else{
      group1.col <- head(brewer.pal(9, "Set1"),length(unique(mapping.sel[,group1])))
      names(group1.col) = levels(as.factor(mapping.sel[,group1]))
...
Go_regression <- function(data, project,  
  ...
  if(!is.null(con.outs)){
    for (con.out in  con.outs) {
      # NA 제거
      data[,con.out] <- as.character(data[[con.out]]);data[,con.out]
      data[,con.out][data[,con.out]==""] <- "NA";data[,con.out]
