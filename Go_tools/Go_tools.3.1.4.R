...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.3.1.5 \n"))
cat("                                              Write by Heekuk \n")
...
Go_lmem <- function(psIN, cate.vars, cate.conf=NULL, StudyID, project, pval=0.05, nsamps_threshold, filt_threshold, taxanames, des, name){
    ...
    for(i in 1:length(taxanames)){
      ...
      for (f in agg[,taxanames[i]]) {
        ...
        df$StudyID <- mapping.sel.cb[df$SampleID, StudyID]
          ...
          #form <- as.formula(sprintf("value ~ %s + %s + (1 | StudyID)", mvar, paste(cate.conf), collapse="-")))
      ...
      write.csv(res.sel, quote = FALSE,col.names = NA,file=sprintf("%s/(%s.vs.%s).Sig%s.%s.%s.%s%s%s.lmem.csv",out_lmem.Tab,
                                                               baseline, 
                                                               smvar,
                                                               dim(res.sel)[1],
                                                               taxanames[i], 
...
# https://bookdown.org/ndphillips/YaRrr/more-colors.html
   print(paste("Also, look the code"," ","piratepal","()", sep="" ))
...
library("yarrr")
  ...
  if(is.null(custumCols) & is.null(RColorBrewer) & is.null(piratepal)){
    ...
    cat("RColorBrewer:","\n", "basel    pony    Xmen    decision    southpark", "\n", "google   eternal   evildead    southpark   google", 
    "\n", "eternal    evildead    bugs    cars    nemo", "\n", " rat    up    espresso    ipodinfo    info2","\n",sep=" ")
  }



#if(RColorBrewer =="All" | RColorBrewer =="all"| RColorBrewer =="ALL"){
#display.brewer.all(type = "qual")
#}else if(piratepal =="All" | piratepal =="all"| piratepal =="ALL"){
#piratepal("all")
#}
  ...
  # for RColorBrewer
  if(is.null(custumCols) & !is.null(RColorBrewer)){
