...
Go_boxplot <- function(df, category.vars, project, outcomes,
                       orders=NULL, 
                       mycols=NULL, 
                       combination=NULL,
                       ylim =NULL,
...
Go_bdiv <- function(psIN, category.vars, project, orders, distance_metrics,
                    plot="PCoA",
                    ellipse="yes",
                    mycols=NULL,
                    combination=NULL,
    ...
    if (!is.null(combination)){
      ...
      for(i in 1:length(group_comparisons)){
        ...
        for(distance_metric in distance_metrics){
          ...
          #===================================#
          if(combination ==2){
            distance <- Go_dist(psIN = psIN.cbn, project = project, distance_metrics = distance_metric)
            
            x <- as.dist(distance[[distance_metric]])
            factors <-  mapping.cbn[,mvar]
            
            co <- combn(unique(as.character(mapping.cbn[,mvar])),2)
            R2 <- c()
            p.value <- c()
            F.Model <- c()
            pairs <- c()
            ...
            # run
            map.pair <- subset(mapping.cbn, mapping.cbn[,mvar] %in% unique(factors))
            ...
            grob <- grobTree(textGrob(paste("PERMANOVA p=",tmp$padj,sep=""), x=0.1,  y=0.95, hjust=0,
