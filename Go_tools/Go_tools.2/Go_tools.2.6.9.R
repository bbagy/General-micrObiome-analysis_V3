...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.2.6.9 \n"))
cat("                                              Write by Heekuk \n")
    ...
    for(oc in outcomes){
      ...
      # paired plot type
       ...
       if (!is.null(paired)) {
        ...
        p1 = p1 + geom_boxplot(aes_string(colour=mvar),outlier.shape = NA)  + theme(legend.position="none")
        
        if (dim(adiv.na)[1] < 500){
          p1 = p1 + geom_jitter(aes_string(colour=mvar),shape=16, alpha = 3, size = 1.5, position=position_jitter(0.2)) # alpha=0.3
        } else{
          p1 = p1
        }
...
Go_deseq2_volc <- function(project, file_path,files, type,alpha,beta, name, height, width){
  ...
  for (fn in 1:length(filenames)) {
    ...
    p1 <- ggplot(data=df.na, aes(x=log2FoldChange, y=-log10(pvalue), colour=dir)) +theme_bw() +
      geom_point(alpha=1, size=2) + scale_color_manual(values=dircolors) +xlab("log2 fold change") + ylab("-log10 (p-value)")+ geom_vline(xintercept = -beta,col = "#1170aa", linetype = "dotted", size = 1) + geom_vline(xintercept = beta,col = "#fc7d0b", linetype = "dotted", size = 1) + theme(plot.title = element_text(size=8), legend.position="bottom", legend.title = element_blank())# + theme()
