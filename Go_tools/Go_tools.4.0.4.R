...
cat(blue("#--------------------------------------------------------------# \n"))
cat(red("                                      Version: Go_tools.4.0.5 \n"))
cat("                                              Write by Heekuk \n")
...
if(relative == T){
  ...
  if (class(ordi.tt) == "try-error"){
    ordering.pc1 <- unique(df2$SampleID)
  }else{
...
Go_rfCMplot <- function(rf, 
  ...
  if(class(rf)=="list"){
    ...
    predicted_classes <- ifelse(rowMeans(predictions_label_test) >= 0.5, unique(testSetDf[,outcome])[1], unique(testSetDf[,outcome])[2])
